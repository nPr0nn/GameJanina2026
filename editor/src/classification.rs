//! Classification-layer label logic: spec, label text, overlap resolution.
//!
//! The actual drawing (outlines + label boxes) is done with egui in
//! [`crate::app`]; this module stays rendering-agnostic.
...
/// Build the `"id | tag"` label string for a classifiable object.
pub(crate) fn build_label_text(id: &str, tag: &str) -> String {
    format!("{id} | {tag}")
}
...
pub(crate) fn resolve_label_overlaps(labels: &mut [LabelSpec]) {
